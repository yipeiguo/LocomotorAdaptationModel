...
% This script is adapted from the file (of the same name) in
% 'simulationCode_revised' folder.
...
condOI = 'Gr43a-40hr'; % '0-125M_24hr' or '0-125M_40hr'
% possible data types:
% 'allTrips_withDistDependence' or 'shortTrips_withDistDependence' or
% 'veryShortTrips' or 'custom'
datatype = 'allTrips_withDistDependence'; 
[runProps,turnProps] = GetSegProps_multipleDataTypes_v2(condOI, datatype);
...
if ifsave == true
    ...
    if isempty(version)
        ...
        fn2save = strcat('SimData','_',condOI,'_',datatype,'_numtrials',num2str(numtrials),'_v',num2str(version));
    end
    foldername = fn2save;
    mkdir(foldername)
end
