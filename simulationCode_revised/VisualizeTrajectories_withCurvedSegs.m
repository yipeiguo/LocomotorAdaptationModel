...
function VisualizeTrajectories_withCurvedSegs(posTrajs, segArcProps_cell,...
    ...
    if exist('ifhitwallVec', 'var') || (ifplotarena == true)
    ...
    for TrajIndx = 1:numTrajs
        ...
        hold on
        scatter(posTraj(1,1),posTraj(2,1),6,'ro','filled'); % exit point
        hold on
        segtype = initsegTypeVec(TrajIndx);
        for segIndx = 1:numsteps
            if segtype == 0
                plot(posTraj(1,segIndx:segIndx+1),posTraj(2,segIndx:segIndx+1),'b-',...
                    'LineWidth',1);
            elseif segtype == 1
        ...
        % plot arena boundary if trajectory hits wall or if desired:
        if exist('ifhitwallVec', 'var')
            ifhitwall = ifhitwallVec(TrajIndx);
        else
            ifhitwall = 0;            
        end
        if (ifhitwall==1) || (ifplotarena==true)
            plot(arena_xscan,arena_yscan,'g');
        end
        xlabel('x');
        ylabel('y');
        if ~exist("subplotTitles",'var')
            title(strcat('traj:',num2str(TrajIndx)));
        else
            title(subplotTitles{TrajIndx});
        end
        axis equal
